...
checkbox({
  ...
  main.variable(observer("viewof curr_date")).define("viewof curr_date", ["Scrubber","d3"], function(Scrubber,d3){return(
...
swatches({
  ...
  main.variable(observer("onMouseOver")).define("onMouseOver", ["d3","code_to_name","x","y"], function(d3,code_to_name,x,y){return(
...
function onMouseOut() {
  ...
  main.variable(observer("draw_date")).define("draw_date", ["d3","curr_date","x","params","y"], function(d3,curr_date,x,params,y){return(
function draw_date(svg) {
  ...
  main.variable(observer("draw_axes")).define("draw_axes", ["xAxis","yAxis","grid"], function(xAxis,yAxis,grid){return(
...
md`## Visualization elements`
  ...
  main.variable(observer("draw_trajectories")).define("draw_trajectories", ["viz_data","color","params","get_path","y_type","onMouseOver","onMouseOut","x","y"], function(viz_data,color,params,get_path,y_type,onMouseOver,onMouseOut,x,y){return(
...
function get_path(d) {
  ...
  main.variable(observer("grid")).define("grid", ["x","params","y"], function(x,params,y){return(
g => g
...
d3.scaleLog([params.ymin, params.ymax], [params.height - params.margin.bottom, params.margin.top])
  ...
  main.variable(observer("xAxis")).define("xAxis", ["params","d3","x","y_type"], function(params,d3,x,y_type){return(
g => g
...
md`## Visualization parameters`
  ...
  main.variable(observer("color")).define("color", ["d3","cntry_data_raw","us_state_data"], function(d3,cntry_data_raw,us_state_data){return(
...
function code_to_name(country_code) {
  ...
  main.variable(observer("viz_data")).define("viz_data", ["data","curr_date"], function(data,curr_date){return(
data.map(function(d) {
  return {
...
data[0].confirmed.length - 1
  ...
  main.variable(observer("data")).define("data", ["cntry_data_raw","us_state_data","selected_regions","add_past_week_number"], function(cntry_data_raw,us_state_data,selected_regions,add_past_week_number){return(
cntry_data_raw.concat(us_state_data).filter(d => selected_regions.includes(d.region)).map(function(d) {
  ...
  main.variable(observer("add_past_week_number")).define("add_past_week_number", ["d3"], function(d3){return(
function add_past_week_number(a) {
  ...
  main.variable(observer("ts_diff")).define("ts_diff", function(){return(
function ts_diff(a) {
  var new_array = [];
  ...
  main.variable(observer("us_state_data")).define("us_state_data", ["us_state_data_raw"], function(us_state_data_raw){return(
us_state_data_raw.map(d => ({
  ...
  main.variable(observer("us_state_data_raw")).define("us_state_data_raw", ["d3"], function(d3){return(
d3.json("https://raw.githubusercontent.com/covid19-data/covid19-data/master/output/us_state_nyt.json")
)});
  main.variable(observer("cntry_data_raw")).define("cntry_data_raw", ["d3"], function(d3){return(
